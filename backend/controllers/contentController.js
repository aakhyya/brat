    ...
    try {
        ...
        const content = await Content.findByIdAndUpdate(
            ...
            updates, //change updates
            {
                ...
                runValidators: true
            } //enforces schema rules on updates
        );
        if (!content) {
            return res.status(404).json({
                success: false,
...
async function rateContent(req, res) {
    try {
        ...
        try {
            ...
            if (user && content && content.featureVector?.length === 30) {
                const updatedVector = tasteVectorService.updateTasteVector(
                    user,
                    content,
                    rating
                );

                user.tasteVector = updatedVector;
                await user.save();
            }
        } catch (err) {
            console.error("Taste vector update failed:", err);
        }
...
async function enrichMovie(req, res) {
    try {
        ...
        // Extract features
        movie.featureVector = featureExtractor.extractFeatures(movie);
...
async function enrichSong(req, res) {
    try {
        ...
        // Extract features
        song.featureVector = featureExtractor.extractFeatures(song);
...
async function enrichBook(req, res) {
    try {
        ...
        // Extract features
        book.featureVector = featureExtractor.extractFeatures(book);
